...
async function main() {
  ...
  console.log(
    ...
    newModels.map((m) => m.key),
  );

  // Generate new lines
  const _newLines = newModels.map(
    (m) => `    "${m.key}": openrouter("${m.id}"),`,
  );

  // Insert into the block, sorted
  const allKeys = Array.from(currentKeys)
    .concat(newModels.map((m) => m.key))
    .sort();
  const newBlockLines = allKeys
    .map((key) => {
      if (currentKeys.has(key)) {
        // Keep existing line
        const existingLine = currentBlock
          .split("\n")
  ...
  const unsupportedMatch = content.match(
    /staticUnsupportedModels = new Set\([^)]*\)/s,
  );
  if (unsupportedMatch) {
